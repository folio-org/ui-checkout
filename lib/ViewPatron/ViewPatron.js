...
class ViewPatron extends React.Component {
  ...
  componentDidMount() {
    const { patron, onSelectPatron, proxiesFor } = this.props;
  ...
  onContinue(data) {
    ...
    const selPatron = (patron.id === data.sponsorId)
      ? patron
      : _.find(proxies, s => s.id === data.sponsorId);
  ...
  render() {
    ...
    const showModal = this.state.showModal;
        console.log("RENDERING", resources.openLoansCount)
    const patronDetail = (
      <div>
        <br />
        <this.connectedPatronDetail id="patron-detail" label="Patron" user={patron} {...this.props} />
    ...
    const proxyDetail = (
      <div>
        <br />
        <this.connectedProxyDetail id="proxy-detail" label="Proxy" user={proxy} {...this.props} />
        <div className={css.section}>
          <Row>
            <Col xs={4}>
