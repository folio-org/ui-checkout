...
class ViewItem extends React.Component {
  ...
  showItemDetails(loan, e) {
    ...
    this.props.parentMutator.query.update({
      _path: `/inventory/view/${loan.item.instanceId}/${loan.item.holdingsRecordId}/${loan.itemId}?query=`,
    });
  }
  ...
  showLoanDetails(loan, e) {
    if (e) e.preventDefault();
    this.props.parentMutator.query.update({
      _path: `/users/view/${loan.userId}?layer=loan&loan=${loan.id}&query=`,
    });
  }
      ...
      <UncontrolledDropdown onSelectItem={this.handleOptionsChange} onToggle={this.onMenuToggle}>
        ...
        <DropdownMenu data-role="menu" pullRight width="10em">
          <MenuItem itemMeta={{ loan, action: 'showItemDetails' }}>
            <Button buttonStyle="dropdownItem" href={`/inventory/view/${loan.item.instanceId}/${loan.item.holdingsRecordId}/${loan.itemId}?query=`}>{this.context.translate('itemDetails')}</Button>
          </MenuItem>
          <MenuItem itemMeta={{ loan, action: 'showLoanDetails' }}>
            <Button buttonStyle="dropdownItem" href={`/users/view/${loan.userId}?layer=loan&loan=${loan.id}&query=`}>{this.context.translate('loanDetails')}</Button>
          </MenuItem>
          {
            this.props.stripes.hasPerm('ui-circulation.settings.loan-policies') &&
            <MenuItem itemMeta={{ loan, action: 'showLoanPolicy' }}>
              <Button buttonStyle="dropdownItem" href={`/settings/circulation/loan-policies/${loan.loanPolicyId}`}>{this.context.translate('loanPolicy')}</Button>
            </MenuItem>
          }
