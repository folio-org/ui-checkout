...
class ViewItem extends React.Component {
  ...
  showLoanDetails(loan, e) {
    if (e) e.preventDefault();
    this.props.parentMutator.query.update({ _path: `/users/view/${loan.userId}`, layer: 'loan', loan: loan.id });
  }
      ...
      <UncontrolledDropdown onSelectItem={this.handleOptionsChange} onToggle={this.onMenuToggle}>
        ...
        <DropdownMenu data-role="menu" pullRight width="10em">
          <MenuItem itemMeta={{ loan, action: 'showItemDetails' }}>
            <Button buttonStyle="dropdownItem" href={`/inventory/view/${loan.item.instanceId}/${loan.item.holdingsRecordId}/${loan.itemId}`}>{this.context.translate('itemDetails')}</Button>
          </MenuItem>
