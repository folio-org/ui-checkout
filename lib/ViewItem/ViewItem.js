...
class ViewItem extends React.Component {
  ...
  getItemFormatter() {
    return {
      Title: loan => _.get(loan, ['item', 'title']),
      'Loan Policy': loan => _.get(loan, ['loanPolicy', 'name']),
      barcode: loan => _.get(loan, ['item', 'barcode']),
      'Due date': loan => (<FormattedDate value={loan.dueDate} />),
      Time: loan => (formatTime(loan.dueDate)),
        ...
        <DropdownMenu data-role="menu" pullRight width="10em">
          ...
          </MenuItem>
          {
            ...
            <MenuItem itemMeta={{ loan, action: 'showLoanPolicy' }}>
              <Button buttonStyle="dropdownItem" href={`/settings/circulation/loan-policies/${loan.loanPolicy.id}`}>Loan policy</Button>
            </MenuItem>
          }
        </DropdownMenu>
      </UncontrolledDropdown>
  ...
  render() {
    ...
    return (
      <MultiColumnList
        ...
        columnWidths={{ no: 28, barcode: 120, Title: 250, 'Loan Policy': 150, 'Due date': 70, Time: 70, ' ': 40 }}
