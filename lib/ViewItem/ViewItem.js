...
const visibleColumns = ['no', 'barcode', 'Instance', 'Loan Policy', 'Due date', 'Time', ' '];
...
class ViewItem extends React.Component {
  ...
  getItemFormatter() {
    return {
      Instance: loan => (<Link to={`/items/view/${loan.itemId}`}>{_.get(loan, ['item', 'title'])}</Link>),
      'Loan Policy': () => '-',
      barcode: loan => (<Link to={`/items/view/${loan.itemId}`}>{_.get(loan, ['item', 'barcode'])}</Link>),
      'Due date': loan => (<FormattedDate value={loan.dueDate} />),
  ...
  showLoanDetails(loan) {
    this.context.history.push(`/users/view/${loan.userId}`);
  }
  ...
  render() {
    ...
    return (
      <MultiColumnList
        ...
        columnWidths={{ no: 28, barcode: 120, Instance: 320, 'Due date': 70, Time: 70, ' ': 40 }}
